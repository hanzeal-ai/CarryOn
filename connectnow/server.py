"""Loopback-only HTTP API and example UI. Run: python3 -m connectnow.server"""
import argparse
import hmac
import json
import os
...
class Handler(BaseHTTPRequestHandler):
    ...
    def auth(self):
        ...
        if not hmac.compare_digest(self.headers.get("Authorization", ""), expected):
            raise BridgeError("请双击 ConnectNow 应用或运行 connectnow open 完成本机配对", 401)
...
def run(port, codex_home, directory):
    ...
    try:
        ...
        lock.close()
        raise ValueError('这个状态目录已有 ConnectNow 实例运行')
...
def main():
    parser = argparse.ArgumentParser(description='ConnectNow 本地 Codex 桥接')
