...
REPOSITORY = 'hanzeal-ai/ConnectNow'
...
def download(url, target, limit):
    ...
    request = urllib.request.Request(url, headers={'User-Agent':'ConnectNow-Updater', 'Accept':'application/vnd.github+json'} if url == LATEST else {'User-Agent':'ConnectNow-Updater'})
    try:
...
def latest(directory):
    ...
    name=f'ConnectNow-{tag.removeprefix("v")}-macos-{arch}-cli.tar.gz'
    assets={}
...
def extract(package, directory):
    ...
    root=directory/'connectnow';seen=set();links=[];size=0
    try:
        with tarfile.open(package,'r:gz') as archive:
            for number,member in enumerate(archive):
                ...
                if path.parts==('._connectnow',) and member.isfile():continue
                if path.is_absolute() or '..' in path.parts or not path.parts or path.parts[0]!='connectnow':
    ...
    binary=next((root/name for name in ('connectnow','ConnectNow') if (root/name).is_file()),None)
    if binary is None or binary.is_symlink() or not os.access(binary,os.X_OK) or not (root/'_internal').is_dir():
        raise ValueError('不是完整的 ConnectNow CLI 安装包')
    return binary
...
def current_link():
    ...
    for candidate in (sys.argv[0],shutil.which('connectnow')):
        if candidate:
            link=Path(candidate).absolute()
            if link.is_symlink() and link.samefile(executable) and link.lstat().st_uid==os.getuid():return link
    raise ValueError('找不到当前 CLI 的安装符号链接；请通过已安装的 connectnow 命令运行')


@contextlib.contextmanager
def installation_lock(link):
    with (link.parent/'.connectnow-maintenance.lock').open('a') as lock:
        fcntl.flock(lock,fcntl.LOCK_EX)
        yield


def replace_link(link, target):
    temporary=link.with_name('.connectnow-'+secrets.token_hex(8))
    try:
...
def service_snapshots():
    ...
    for row in list_services()['services']:
        ...
        if (not Path(executable).is_absolute() or not Path(executable).is_file()
                or Path(executable).name not in ('connectnow','ConnectNow','connectnow-service')):
...
def install(package, link, releases, restart=False):
    match=re.fullmatch(r'ConnectNow-(\d+\.\d+\.\d+)-macos-(arm64|x86_64)-cli\.tar\.gz',package.name)
...
def update(package=None, check=False):
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='connectnow-download-') as temp:
            if package is None:
            ...
            target,previous=install(package,link,Path.home()/'.local/share/connectnow-releases',restart=True)
