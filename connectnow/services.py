...
def catalog_directory():
    override=os.environ.get('CONNECTNOW_REGISTRY_DIR')
...
def process_directories():
    ...
    for line in output.splitlines():
        if not re.search(r'(?:^|/|\s)(?:connectnow|ConnectNow|connectnow-service)(?:\s+serve|\.server\s)',line):continue
...
def list_services(current=None):
    ...
    extra={path:{'directory':path,'name':'默认工作区' if Path(path)==Path.home()/'Library/Application Support/ConnectNow' else Path(path).name}
