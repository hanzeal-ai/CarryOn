    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(label)
                Spacer()
                if case .number(let used) = window["usedPercent"] { Text("已用 \(used, specifier: "%.0f")%").monospacedDigit() }
                else { Text("用量未知").foregroundStyle(Design.secondary) }
            }.font(.caption)
            if case .number(let used) = window["usedPercent"] {
                ProgressView(value: min(100, max(0, used)), total: 100).tint(used >= 90 ? .orange : Design.ink)
            }
            if case .number(let reset) = window["resetsAt"] {
