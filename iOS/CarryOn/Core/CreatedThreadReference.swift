...
public enum CreatedThreadReference {
    private static let uuid = #"[0-9a-fA-F]{8}(?:-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12}"#
    private static let pattern = #"(?m)^::created-thread\{(?:(threadId)="("# + uuid + #")"|(clientThreadId)="((?:client-new-thread:)?"# + uuid + #")")\}[ \t]*$"#
    ...
    public static func render(_ text: String, titles: [String: String]) -> String {
        ...
        for match in regex.matches(in: text, range: NSRange(text.startIndex..., in: text)).reversed() {
            ...
            if match.range(at: 3).location != NSNotFound {
                result.replaceSubrange(range, with: "会话创建中")
                continue
            }
            guard let idRange = Range(match.range(at: 2), in: text) else { continue }
