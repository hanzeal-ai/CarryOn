...
with tempfile.TemporaryDirectory(prefix='carryon-desktop-') as temp:
    ...
    subprocess.run(['xcrun','swiftc','-parse-as-library','-swift-version','5','-D','DESKTOP_TEST',
        '-target',platform.machine()+'-apple-macos13.0',*map(str,sorted((ROOT/'desktop').glob('*.swift'))),str(ROOT/'iOS/CarryOn/Core/AppUpdate.swift'),str(ROOT/'tests/DesktopSmoke.swift'),'-o',str(binary)],check=True)
    try:
