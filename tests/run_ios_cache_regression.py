"""Run the real cache UI/model against an isolated, controllable HTTP transport."""
import json
...
if len(sys.argv) != 2:
    raise SystemExit("Usage: python3 tests/run_ios_cache_regression.py <booted-simulator-id>")
repo = Path(__file__).resolve().parents[1]
work = repo / ".runtime/cache-regression"
source = work / "source"
...
shutil.copy2(repo / "tests/fixtures/ios_cache_regression.swift", source / "CarryOn/App/CarryOnApp.swift")
shutil.copy2(repo / "tests/fixtures/ios_cache_transport.swift", source / "CarryOn/Core/CacheProtocol.swift")
...
api_text = api.read_text().replace(transport_default, "configuration: URLSessionConfiguration = CacheProtocol.configuration")
start = api_text.index('        guard let cookie else', api_text.index('public func stream('))
end = api_text.index('\n    public func restoreSession()', start)
api_text = api_text[:start] + '        return try await CacheSocket.connect(selection)\n    }\n' + api_text[end:]
api.write_text(api_text)
project = source / "CarryOn.xcodeproj/project.pbxproj"
project.write_text(project.read_text().replace("com.hanzeal.carryon", "com.hanzeal.carryon.cacheregression"))
...
for _ in range(150):
    ...
    time.sleep(0.2)
raise SystemExit("Cache regression did not produce a result within 30 seconds")
