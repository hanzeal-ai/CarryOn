...
try{for(const width of [1280,390]){
 const page=await browser.newPage({viewport:{width,height:844}});page.on('pageerror',e=>{errors.push(e.message);console.error('PAGE ERROR',e.message)});let requests=[],mode='idle',delay=0;
 ...
 socket.send(JSON.stringify({type:'update',threadId:m.threadId,subscription:m.subscription,status:{enabled:true,controllerId:P},jobs:[],...(m.threadId?{history:history(P)}:{}),...(m.includeSideChats?{sideChats:{chats:[{id:C,title:'侧边一',label:'空闲'},{id:D,title:'侧边二',label:'空闲'}]},sideThreadId:m.sideThreadId,...(m.sideThreadId?{sideHistory:history(m.sideThreadId,mode)}:{})}:{})}));}));
 ...
 mode='running';await page.evaluate(()=>loadHistory());await page.waitForFunction(()=>sideComposeHistory?.status.state==='running');await page.locator('#side-prompt').fill('');await page.locator('#side-send').click();await page.waitForFunction(()=>!sideSending);
