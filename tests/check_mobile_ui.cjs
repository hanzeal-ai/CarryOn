...
(async()=>{
 ...
 await page.locator('.bottom-nav').getByRole('button',{name:'设置',exact:true}).click();await page.screenshot({path:'.runtime/mobile-settings.png'});await compare('devices',[['.device-card','.device-card'],['.workspace-card-title','.workspace-card-title'],['.workspace-settings .setting','.workspace-settings .setting'],['.bottom-nav','.bottom-nav']]);
 ...
 await page.evaluate(()=>{fixture.runtime={type:'idle'};fixture.status={state:'idle',label:'空闲'};fixture.controls.lastUserText='原有任务内容';fixture.controls.lastTurnId='last';fixture.controls.requests=[];Operations.render(fixture,selected,client,notice,true);});
 await page.getByRole('button',{name:'会话工具',exact:true}).click();await page.getByRole('button',{name:'编辑最后一轮',exact:true}).click();assert.equal(await page.locator('#mobile-edit textarea').inputValue(),'原有任务内容');await page.screenshot({path:'.runtime/mobile-edit.png'});await page.locator('#mobile-edit button[aria-label="返回"]').click();
 ...
 await local.close();
 assert.deepEqual(errors,[]);console.log('Matched design geometry:',visualChecks.length);console.log('PASS design components: navigation, full screen create/edit, chat, popover, settings, requests, draft/read gates, desktop restoration, queue management, message clipboard, local settings placement and narrow/short viewports. API responses mocked.');await browser.close();
