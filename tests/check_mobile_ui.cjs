...
(async()=>{
 ...
 const visualChecks=[];
 async function compare(route,pairs,yOffset=0,heightOffset=0){
  await reference.evaluate(value=>{location.hash=value},route);await reference.waitForTimeout(60);
  for(const [actual,expected]of pairs){const a=await page.locator(actual).first().boundingBox(),b=await reference.locator(expected).first().boundingBox();assert(a&&b,actual+' exists');for(const key of ['x','y','width','height'])assert(Math.abs(a[key]-(b[key]+(key==='y'?yOffset:key==='height'?heightOffset:0)))<=2,actual+' '+key+': '+a[key]+' vs '+b[key]);visualChecks.push(actual);}
 }
 ...
 await page.getByRole('button',{name:'会话工具',exact:true}).click();await page.screenshot({path:'.runtime/mobile-tools.png'});await reference.evaluate(()=>showFeature('operations'));// Additional functional rows keep the design row height and bottom-sheet alignment.
 const toolRowHeight=(await reference.locator('#sheet .setting').first().boundingBox()).height;assert.equal((await page.getByRole('button',{name:'队列管理',exact:true}).boundingBox()).height,toolRowHeight);const additionalRows=await page.locator('#mobile-tools .setting').count()-await reference.locator('#sheet .setting').count();await compare('chat/1',[['#mobile-tools .sheet-top','#sheet .sheet-top'],['#mobile-tools .setting','#sheet .setting']],-additionalRows*toolRowHeight);await reference.locator('#sheet button[aria-label=关闭]').click();await page.locator('#mobile-tools button[aria-label="关闭"]').click();
 ...
 for(const selector of ['.message.user','.message.assistant']){
  const message=page.locator(selector).first();await message.getByRole('button',{name:'复制原文',exact:true}).click();await message.getByRole('button',{name:'已复制',exact:true}).waitFor();
 ...
 await page.locator('.bottom-nav').getByRole('button',{name:'我的',exact:true}).click();await page.screenshot({path:'.runtime/mobile-settings.png'});await compare('devices',[['.device-card','.device-card']],0,2*toolRowHeight);await compare('devices',[['.workspace-card-title','.workspace-card-title'],['.bottom-nav','.bottom-nav']]);await compare('devices',[['.workspace-settings>div.group>.setting','.workspace-settings>div.group>.setting']],2*toolRowHeight);
 ...
 await local.setViewportSize({width:1440,height:1000});await local.locator('body>header #bridge').waitFor();
 assert((await local.locator('body>header #bridge').textContent()).includes('CLI 或桌面端'));
 assert.equal(await local.getByRole('button',{name:'停止本地服务',exact:true}).count(),0);
 await local.setViewportSize({width:390,height:844});await local.locator('body>header #bridge').waitFor({state:'hidden'});assert(!(await local.locator('#bridge').isVisible()));
 await local.close();
