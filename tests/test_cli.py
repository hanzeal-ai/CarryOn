...
class CLITests(unittest.TestCase):
    def test_cloud_connect_requests_confirmation_for_each_state_directory(self):
        with tempfile.TemporaryDirectory() as temp:
            for name,control in [('a',False),('b',True)]:
                ...
                output=io.StringIO()
                args=['cloud','connect','--state-dir',str(state),'--url','https://example.test/connectnow']
                if control:args.append('--allow-control')
                with patch('connectnow.cli.running',return_value={'port':1234}), patch('connectnow.cli.call',return_value={
                        'id':'request-id','verification':'ABC123','url':'https://example.test/connectnow/#connect=request-id'}) as call, \
                        patch('connectnow.cli.getpass.getpass') as prompt, redirect_stdout(output):
                    self.assertEqual(main(args),0)
                call.assert_called_once_with(state.resolve(),'/cloud/link/start',
                    {'url':'https://example.test/connectnow','control':control},timeout=20)
                prompt.assert_not_called()
                self.assertIn('ABC123',output.getvalue())
                self.assertIn('https://example.test/connectnow/#connect=request-id',output.getvalue())
    ...
    def test_cloud_connect_requires_running_service_and_valid_options(self):
        for running,options,error in [(None,['--url','https://example.test'],'服务未运行'),
            ...
            with self.subTest(options=options), patch('connectnow.cli.running',return_value=running), \
                    patch('connectnow.cli.call') as call, patch('connectnow.cli.getpass.getpass') as prompt, \
    ...
    def test_cloud_connect_request_error_does_not_claim_success(self):
        with patch('connectnow.cli.running',return_value={'port':1234}), \
                patch('connectnow.cli.call',side_effect=ValueError('云端连接授权失败')), \
    ...
    def test_cloud_connect_existing_credentials_remain_explicit(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch('connectnow.cli.running',return_value={'port':1234}), \
                    patch('connectnow.cli.call',return_value={}) as call, redirect_stdout(io.StringIO()):
    ...
    def test_start_reuse_assets_and_authenticated_stop(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            env=dict(os.environ,CONNECTNOW_REGISTRY_DIR=str(Path(temp)/'registry'))
    ...
    def test_settings_commands_share_service_routes_and_binding_identity(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            for arguments,path,body in cases:
                with self.subTest(arguments=arguments), patch('connectnow.cli.running',return_value={'port':1234}), \
                        patch('connectnow.cli.call',return_value={}) as call, redirect_stdout(io.StringIO()):
            ...
            for arguments in [['cloud','control'],['controller','set']]:
                with patch('connectnow.cli.running',return_value={'port':1234}), patch('connectnow.cli.call') as call, redirect_stderr(io.StringIO()):
    ...
    def test_notification_change_preserves_other_preferences(self):
        ...
        with patch('connectnow.cli.running',return_value={'port':1234}), \
                patch('connectnow.cli.call',side_effect=[{'preferences':prefs},{}]) as call, redirect_stdout(io.StringIO()):
