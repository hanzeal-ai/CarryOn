...
install_dir="${CARRYON_INSTALL_DIR:-$HOME/.local/share/carryon}"
bin_dir="${CARRYON_BIN_DIR:-$HOME/.local/bin}"
if [ ! -x "$source_dir/carryon" ] || [ ! -d "$source_dir/_internal" ]; then
...
if [ -e "$bin_dir/carryon" ] || [ -L "$bin_dir/carryon" ]; then
  echo "$bin_dir/carryon 已存在，请先核对已有安装。" >&2
  exit 1
fi
