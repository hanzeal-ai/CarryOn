...
本机输入云端 HTTPS 地址并申请连接；登录云端的管理员在「连接申请」核对设备名称、时间和两端确认码，点击确认后自动登记设备。本机后台领取独立设备凭证，开启桥接并建立出站 WSS。无需预先填写设备 ID/Token，不需要端口映射。
...
1. 启动本机 ConnectNow，展开「连接与本机设置」，填写云端地址、选择是否允许远程控制，点击「连接云端」。
2. 云端「连接申请」按钮显示待处理数量，点击后核对确认码，确认或拒绝。确认后无需选择预登记设备。
3. 本机后台自动绑定。关闭本地网页不终止申请；本机服务停止或申请超过五分钟时需重新申请。
