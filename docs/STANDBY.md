...
在桌面端打开「远程待机」，或执行 `connectnow standby on`；关闭使用 `connectnow standby off`，查询使用 `connectnow standby status`。默认关闭；选择会保存在用户状态目录的 `standby.json`，服务下次启动时恢复。无需 sudo，不修改系统的永久睡眠设置。
