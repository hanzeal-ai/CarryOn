...
在本机 example 的「云端接入与本地服务」中填写控制台地址，点击「连接云端」。浏览器打开云端后登录，核对两边显示的确认码、选中的设备，再点「确认连接」。本机会自动完成绑定并开启桥接；浏览器没有打开时使用页面上的「打开云端确认连接」链接。授权请求 5 分钟过期，仅可兑换一次，轮询秘密与设备 Token 不进入浏览器。

已连接时，勾选或取消「允许远程控制」，点击「保存权限」即可，无需重新配对。此操作会短暂重连设备通道；云端不能改变本机权限。保留「使用一次性配对码」作为备用连接方式。
