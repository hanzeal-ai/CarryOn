...
在本机 example 的「云端接入与本地服务」中填写控制台地址，点击「连接云端」。登录云端后在「连接申请」查看通知，核对两边显示的确认码，再点「确认连接」。本机会自动完成绑定并开启桥接；浏览器没有打开时使用页面上的「打开云端确认连接」链接。授权请求 5 分钟过期，仅可兑换一次，轮询秘密与设备 Token 不进入浏览器；确认后的凭证可由同一申请秘密在有效期内重试领取。
