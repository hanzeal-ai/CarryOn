...
class Gateway(ThreadingHTTPServer):
    ...
    def __init__(self,address,config,handler=None):
        ...
        self.release=os.environ.get("CARRYON_RELEASE","development")
    ...
    def handle_api(self,method):
        try:
            ...
            if parsed.path=='/healthz' and method=='GET':
                self.reply(200,{'service':'carryon-gateway','version':__version__,'release':self.server.release});return
...
def main():
    parser=argparse.ArgumentParser(description='CarryOn 自托管参考网关（置于 TLS 反向代理后）')
