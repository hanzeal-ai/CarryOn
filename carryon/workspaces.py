...
def backend(directory):
    return records().get(str(Path(directory).resolve()), {}).get('backend', 'ipc')
...
def initialize(directory):
    ...
    with (root/'workspace-creation.lock').open('a+') as lock:
        ...
        existing = records()
        if str(directory) in existing and not existing[str(directory)].get('removed'):
            return existing[str(directory)]
        ipc = next((row for row in existing.values() if not row.get('removed') and row.get('backend','ipc') == 'ipc'), None)
        if ipc:
